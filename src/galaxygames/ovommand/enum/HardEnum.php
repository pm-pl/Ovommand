...
class HardEnum extends BaseEnum implements IStaticEnum{
	...
	public function addAliases(array $aliases, bool $isHidden = false) : void{
		if (Server::getInstance()->getTick() !== 0) { // hacky method for checking if the server has started
			throw new EnumException(Messages::EXCEPTION_ENUM_RUNNING_HARD_ENUM_ADD_ALIAS->translate(['enumName' => $this->getName()]), EnumException::ENUM_EDIT_RUNNING_HARD_ENUM);
		}
	...
	public function removeAliases(array $aliases, bool $isHidden = false) : void{
		if (Server::getInstance()->getTick() !== 0) { // hacky method for checking if the server has started
			throw new EnumException(Messages::EXCEPTION_ENUM_RUNNING_HARD_ENUM_REMOVE_ALIAS->translate(['enumName' => $this->getName()]), EnumException::ENUM_EDIT_RUNNING_HARD_ENUM);
		}
	...
	public function removeValues(array $keys) : void{
		if (Server::getInstance()->getTick() !== 0) {
			throw new EnumException(Messages::EXCEPTION_ENUM_RUNNING_HARD_ENUM_REMOVE_VALUE->translate(['enumName' => $this->getName()]), EnumException::ENUM_EDIT_RUNNING_HARD_ENUM);
		}
		$updates = [];
		foreach ($keys as $k) {
	...
	public function addValues(array $values, array $showAliases = [], array $hiddenAliases = []) : void{
		if (Server::getInstance()->getTick() !== 0) {
			throw new EnumException(Messages::EXCEPTION_ENUM_RUNNING_HARD_ENUM_ADD_VALUE->translate(['enumName' => $this->getName()]), EnumException::ENUM_EDIT_RUNNING_HARD_ENUM);
		}
		$updates = [];
	...
	public function changeValue(string $key, mixed $value) : void{
		if (Server::getInstance()->getTick() !== 0) {
			throw new EnumException(Messages::EXCEPTION_ENUM_RUNNING_HARD_ENUM_CHANGE_VALUE->translate(['key' => $key, 'enumName' => $this->getName()]), EnumException::ENUM_EDIT_RUNNING_HARD_ENUM);
		}
